...
start)
	...
	xterm -fg red -title "SSH: dange@localhost" -e 'echo;echo "Starting remote SSH session at dange@localhost ..."; echo;echo;ssh -X dange@localhost "cd /tmp/SystemP3dxNavigationPlayerStageSimulator.deployment; pwd; bash --login -i -e start-PC1.sh start"' &
	echo $! >> $PID_FILE
...
stop)
	...
	xterm -title "stopping dange@localhost" -e 'echo;echo "Starting remote SSH session at dange@localhost ..."; echo;echo;ssh -X dange@localhost "cd /tmp/SystemP3dxNavigationPlayerStageSimulator.deployment; pwd; bash --login -i -e start-PC1.sh stop"' 
	echo $! >> $PID_FILE
