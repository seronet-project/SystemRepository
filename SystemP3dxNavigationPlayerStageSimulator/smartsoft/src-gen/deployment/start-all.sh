...
start)
	...
	xterm -fg red -title "SSH: localhost" -e 'echo;echo "Starting remote SSH session at localhost ..."; echo;echo;ssh -X localhost "cd /tmp/SystemP3dxNavigationPlayerStageSimulator.deployment; pwd; bash --login -i -e start-PC1.sh start"' &
	echo $! >> $PID_FILE
...
stop)
	...
	xterm -title "stopping localhost" -e 'echo;echo "Starting remote SSH session at localhost ..."; echo;echo;ssh -X localhost "cd /tmp/SystemP3dxNavigationPlayerStageSimulator.deployment; pwd; bash --login -i -e start-PC1.sh stop"' 
	echo $! >> $PID_FILE
