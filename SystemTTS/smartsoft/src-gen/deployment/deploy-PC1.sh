...
DEPLOYMENT_DIRECTORY=SystemTTS.deployment/
...
source src/predeploy-ComponentTTS.sh 2>&1
...
DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_ComponentTTS/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS $REFERENCED_PROJECT_ComponentTTS/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS $I"
	fi
done
...
DEPLOY_LIBRARIES=""
DEPLOY_COMPONENT_FILES=""
source src/predeploy-ComponentTTSClient.sh 2>&1
...
DEPLOY_COMPONENT_FILES_PATHS_ComponentTTSClient=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_ComponentTTSClient/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTTSClient="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTTSClient $REFERENCED_PROJECT_ComponentTTSClient/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTTSClient="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTTSClient $I"
	fi
done
...
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS $TMPDIR/ComponentTTS_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_ComponentTTS" = "" ]; then
